...
async def delete_source(
    ...
    db.delete(source)
    db.commit()
    # message_count is denormalized; the cascade removed alert_messages links,
    # so recompute affected alerts to keep the queue truthful
    db.execute(
        text(
            "UPDATE alerts SET message_count = (SELECT count(*) FROM alert_messages "
            "WHERE alert_messages.alert_id = alerts.id) WHERE source_id = :sid"
        ),
        {"sid": source_id},
    )
    db.commit()
