...
def _sanitize_filename(name: str | None) -> str | None:
    """MEDIA-01: keep only ASCII-printable, quote-free filename chars so a
    Telegram-controlled filename can never inject header values."""
    if not name:
        return None
    cleaned = re.sub(r"[^\x20-\x7e]", "_", name).replace('"', "_").replace("\\", "_")
...
def download_media(message_id: int):
    ...
    try:
        ...
        if (msg.media_attempts or 0) >= 3:
            return  # terminal: give up after 3 failed attempts (MEDIA-04)
        if msg.media_type not in _ALLOWED_MEDIA_TYPES:
            return
        meta = msg.media_metadata or {}
        mime = str(meta.get("mime_type", "")).lower()
        if msg.media_type == "document" and (not mime.startswith("image/") or "svg" in mime):
            return
        # never buffer an oversized document (MEDIA-02)
        if msg.media_type == "document" and int(meta.get("size") or 0) > settings.media_max_bytes:
            return
        from ..services.collector_state import get_service
        ...
        msg.media_filename = _sanitize_filename(result.get("filename"))  # MEDIA-01
        msg.media_stored = True
        msg.media_attempts = 0
        db.commit()
        ...
        if msg is not None:
            ...
            if (msg.media_attempts or 0) >= 3:
                msg.media_stored = False  # terminal failure — stop retrying
                db.commit()
                logger.error("media download permanently failed", extra={"message_id": message_id, "error": str(e)})
                return
            db.commit()
        logger.warning("media download failed", extra={"message_id": message_id, "error": str(e), "attempts": msg.media_attempts if msg else None})
        raise _db_retry(30)
    finally:
        db.close()
...
def media_backfill_missing_task():
    ...
    try:
        ...
        for mid in rows:
            enqueue(TASK_MEDIA_DOWNLOAD, message_id=mid, queueing_lock=f"media:{mid}")
        if rows:
