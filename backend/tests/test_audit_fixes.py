...
from .conftest import ADMIN_PASSWORD, login_admin, login_operator
...
def test_token_version_increments_on_password_change(client):
    ...
    r = client.post(
        ...
        json={"current_password": ADMIN_PASSWORD, "new_password": "a-new-admin-pass-1"},
    )
    assert r.status_code == 200, r.text
    client.cookies.clear()
    ...
    r = client.post(
        "/api/v1/auth/password",
        json={"current_password": "a-new-admin-pass-1", "new_password": ADMIN_PASSWORD},
    )
    assert r.status_code == 200, r.text
